...
\`npx properui@latest info --json\`, search the registry with \`npx properui@latest search\`,
and prefer \`npx properui@latest add\` (or \`add example <name>\` for a whole screen) over
...
1. Run \`npx properui@latest info --json\` to see what is already configured and installed.
2. Search the registry (\`npx properui@latest search "<what you need>"\`) before writing markup
...
- Run \`npx properui@latest info --json\` to see the project's Proper UI setup and what is
  already installed.
- Run \`npx properui@latest search "<what you need>"\` to check whether a component or
  full-page example already covers it.
- Install matches with \`npx properui@latest add <name>\` (or \`add example <name>\` for a whole
