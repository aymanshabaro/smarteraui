...
export type GroupProps = ComponentPropsWithRef<typeof OTPInput> & {
    width?: number;
...
const FakeCaret = ({ size = "md" }: { size?: PinInputSize }) => {
    return <div className={cx("animate-caret-blink bg-fg-brand-primary pointer-events-none h-[1em] w-0.5", styles[size].caret)} />;
};

const Separator = (props: ComponentPropsWithRef<"div">) => {
    return (
        <div role="separator" {...props} className={cx("text-display-xl text-utility-neutral-300 text-center font-medium", props.className)}>
            -
        </div>
    );
};
