...
import type { ComboBoxProps as AriaComboBoxProps, GroupProps as AriaGroupProps, Key as AriaKey, ListBoxProps as AriaListBoxProps } from "react-aria-components";
import {
...
const TagSelectContext = createContext<{
    ...
    selectedItems: AriaListData<SelectItemType>;
    onRemove: (keys: Set<AriaKey>) => void;
    onInputChange: (value: string) => void;
    valueFormatter?: (item: SelectItemType) => string;
}>({
    selectedKeys: [],
    selectedItems: {} as AriaListData<SelectItemType>,
    onRemove: () => {},
    onInputChange: () => {},
});
...
interface TagSelectProps extends Omit<AriaComboBoxProps<SelectItemType>, "children" | "items">, RefAttributes<HTMLDivElement> {
    ...
    onItemCleared?: (key: AriaKey) => void;
    onItemInserted?: (key: AriaKey) => void;
    valueFormatter?: (item: SelectItemType) => string;
}
...
export const TagSelectBase = ({
    ...
    const accessibleList = AriaUseListData({
        initialItems: items,
        filter,
    });
    ...
    const onSelectionChange = (id: AriaKey | null) => {
        if (!id) {
            return;
        }
...
const InnerTagSelect = ({ isDisabled, shortcut, shortcutClassName, placeholder, size = "sm" }: Omit<TagSelectProps, "selectedItems" | "children">) => {
    ...
    const comboBoxStateContext = useContext(AriaComboBoxStateContext);
    ...
    const handleTagKeyDown = (event: KeyboardEvent<HTMLButtonElement>, value: AriaKey) => {
        ...
        >
            {({ isDisabled }) => (
                <>
                    ...
                    {/* eslint-disable-next-line jsx-a11y/no-autofocus -- explicitly disables FocusScope's own autofocus-on-mount behavior; nothing is auto-focused on page load. */}
