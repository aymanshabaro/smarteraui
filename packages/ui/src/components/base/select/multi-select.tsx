...
const MultiSelectFooter = ({ size = "sm", onReset, onSelectAll, className }: MultiSelectFooterProps) => {
    ...
    return (
        <div className={cx("border-secondary flex items-center justify-between border-t p-3", className)}>
                    ...
                    <AriaButton
                        ...
                        className={(state) =>
                            cx(
                                ...
                                (state.isFocusVisible || state.isPressed) && "ring-brand ring-2",
                        ...
                        >
                            ...
                            {hasSelection ? (
                                <span className={cx("flex items-center", sizes[size].textContainer)}>
                                    <span className={cx("text-primary font-medium", sizes[size].text)}>
                    ...
                    >
                        <AriaDialog className="outline-hidden">
                            <AriaAutocomplete filter={contains} inputValue={searchValue} onInputChange={setSearchValue}>
                                {showSearch && (
                                    <div className={cx("border-secondary border-b", searchSizes[size].wrapper)}>
                                        {/* eslint-disable-next-line jsx-a11y/no-autofocus -- autofocus targets the search field inside a popover the user just opened by clicking the trigger, not on page load. */}
                                        <AriaSearchField aria-label="Search" value={searchValue} onChange={setSearchValue} autoFocus>
                                            <div className={cx("flex items-center", searchSizes[size].root)}>
                                                <SearchLg data-icon aria-hidden="true" className="text-fg-quaternary shrink-0" />
                                                <AriaInput
                                                    placeholder="Search"
                                                    className={cx(
