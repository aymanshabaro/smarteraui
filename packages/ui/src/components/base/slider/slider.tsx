...
interface SliderProps extends AriaSliderProps {
    /** Where the current value label is rendered relative to the thumb. */
    labelPosition?: keyof typeof styles;
    /** Formats the value shown in the thumb label. Defaults to a rounded percentage. */
...
export const Slider = ({ labelPosition = "default", minValue = 0, maxValue = 100, labelFormatter, formatOptions, ...rest }: SliderProps) => {
    ...
    // Render an empty (visually hidden) <Label> only when the consumer has not already
    // supplied an accessible name — an always-empty <label> would otherwise fail axe's
    // "form elements must have labels" rule.
