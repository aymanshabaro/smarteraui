...
type FeaturedIconBaseProps = ComponentPropsWithRef<typeof FeaturedIconbase>;
...
const FeaturedIcon = ({
    ...
    size,
    ...props
}: Omit<FeaturedIconBaseProps, "color"> & { color?: FeaturedIconBaseProps["color"] }) => {
...
const Illustration = ({ type = "cloud", size = "lg", ...props }: ComponentPropsWithRef<typeof Illustrations>) => {
    const { size: rootSize } = useContext(RootContext);

    return (
        // Decorative by default — the empty state's Title/Description already carry the meaning.
        // Callers wanting it announced can override by passing their own `aria-hidden`/`aria-label`.
        <Illustrations
            aria-hidden="true"
            {...props}
            {...{ type }}
        ...
        >
            ...
            {avatars.slice(0, avatarSlots.length).map((avatar, i) => {
                ...
                return (
                    <div
                        key={i}
                        className={cx("bg-primary absolute top-1/2 left-1/2 rounded-full p-px shadow-xs ring-[0.5px] ring-black/10", slot.size)}
...
const gridStyles = {
    sm: { avatar: "size-8 rounded-md", inner: "rounded-[5px]", gap: "gap-2 ps-2", rowGap: "gap-2" },
    md: { avatar: "size-10 rounded-lg", inner: "rounded-[7px]", gap: "gap-3 ps-3", rowGap: "gap-3" },
    lg: { avatar: "size-12 rounded-[10px]", inner: "rounded-[9px]", gap: "gap-3 ps-3", rowGap: "gap-3" },
} as const;
...
const AvatarGrid = ({ avatars = [], ...props }: AvatarGridProps) => {
    ...
    const renderGridAvatar = (avatar: { src: string; alt?: string }, key: number) => (
        <div key={key} className={cx("bg-primary shrink-0 p-px shadow-xs ring-[0.75px] ring-black/10", config.avatar)}>
            <div
                className={cx(
