...
const CarouselRoot = ({ orientation = "horizontal", opts, setApi, plugins, className, children, ...props }: ComponentPropsWithRef<"div"> & CarouselProps) => {
    ...
    useEffect(() => {
        ...
        // Embla does not emit an initial "select"/"reInit" event, so the first sync must run
        // synchronously here to seed scroll-snap/trigger state before the first paint.
