...
const CustomRadarChartTick = ({ payload, x, y, textAnchor, stroke }: BaseTickContentProps) => {
    ...
    useEffect(() => {
        // `getBBox` is unimplemented in jsdom (used by the vitest a11y smoke tests); guard so the
        // demo still renders there. Real browsers always provide it on SVG text elements.
        if (textRef.current && rectRef.current && typeof textRef.current.getBBox === "function") {
    ...
    return (
        <ResponsiveContainer initialDimension={{ width: 1, height: 1 }} height={500} width="100%">
            <RechartsRadarChart
                ...
                outerRadius="80%"
                data={radarData}
                className="text-tertiary [&_.recharts-polar-grid]:text-utility-neutral-100 size-full font-medium [&_.recharts-text]:text-sm"
                margin={{
                    left: 0,
                    right: 0,
                    top: 0,
...
// The hero preview at the top of the docs page reuses the radar chart.
export const RadarChartExample = RadarChart;
