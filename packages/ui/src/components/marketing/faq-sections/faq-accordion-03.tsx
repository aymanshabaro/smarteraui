...
const styles = sortCx({
    ...
    trigger:
        "flex w-full cursor-pointer items-start justify-between gap-6 rounded-md text-start outline-focus-ring select-none focus-visible:outline-2 focus-visible:outline-offset-2 md:gap-6",
...
/** A two-column layout with the heading beside a plain accordion. */
        ...
        <div className="max-w-container mx-auto px-4 md:px-8">
            <div className="flex flex-col gap-12 lg:flex-row lg:gap-16">
                <div className="flex w-full max-w-3xl flex-col lg:max-w-xl">
                    ...
                    <h2 className="text-display-sm text-primary md:text-display-md mt-3 font-semibold">FAQs</h2>
                ...
                <AriaDisclosureGroup defaultExpandedKeys={[faqs[0].id]} className="flex w-full flex-col gap-8">
                    {faqs.map((faq) => (
                        <AriaDisclosure key={faq.id} id={faq.id}>
                            {({ isExpanded }) => (
                                <>
                                    <AriaHeading level={3}>
