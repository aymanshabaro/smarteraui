...
const styles = sortCx({
    ...
    tabLogo: "h-8 object-contain md:h-10 dark:invert",
});
...
const reviews = [
    ...
    },
    {
        id: "review-04",
        logo: LOGOS[3],
        quote: "We've really sped up our workflow using Proper UI and haven't looked back since.",
        author: AVATARS[8],
        role: "Data Engineer",
    },
    {
        id: "review-05",
        logo: LOGOS[4],
        quote: "Every project starts with Proper UI, and it's made a huge difference to our output.",
        author: AVATARS[10],
        role: "Web Developer",
    },
] as const;

/** A centered quote whose customer logos double as tabs, with prev/next controls on mobile. */
                    ...
                    <div className="flex flex-col items-center gap-10 md:gap-12">
                        ...
                        {reviews.map((item) => (
                            <AriaTabPanel key={item.id} id={item.id} className="flex flex-col text-center">
                                <figure className="flex flex-col gap-8">
                                    <blockquote className="text-display-sm md:text-display-md lg:text-display-lg text-primary font-medium text-balance">
                                        {item.quote}
                                    </blockquote>

                                    <figcaption className="flex justify-center">
                                        <div className="flex flex-col items-center gap-4">
                                            <Avatar size="lg" border src={item.author.src} alt={item.author.name} />

                                            <div className="flex flex-col gap-1">
                                                <p className="text-primary text-lg font-semibold">{item.author.name}</p>
                        ...
                        <div className="flex w-full items-center justify-between md:hidden">
                            <Button color="link-color" size="sm" aria-label="See previous review" onPress={() => go(-1)}>
                                <ArrowNarrowLeft aria-hidden="true" className="text-quaternary size-6" />
                            </Button>

                            <img src={review.logo.src} alt={review.logo.name} className="h-10 object-contain dark:invert" />

                            <Button color="link-color" size="sm" aria-label="See next review" onPress={() => go(1)}>
                                <ArrowNarrowRight aria-hidden="true" className="text-quaternary size-6" />
                            </Button>
                        </div>
                    </div>
                </div>
