...
const styles = sortCx({
    /** The tinted card each member sits in. */
    card: "flex flex-col items-center gap-5 bg-secondary px-6 py-6 md:pe-4",
...
/** A centered intro with calls to action above a grid of tinted profile cards. */
        ...
        <div className="max-w-container mx-auto px-4 md:px-8">
            ...
            <div className="mt-12 md:mt-16">
                <ul className="grid w-full grid-cols-1 gap-x-8 gap-y-6 sm:grid-cols-2 md:gap-y-8 lg:grid-cols-3 xl:grid-cols-4">
                    {team.map((member) => (
                        <li key={member.photo.name} className={styles.card}>
                            <Avatar size="2xl" border src={member.photo.src} alt={member.photo.name} className="size-24" />
