...
This is `bench/` from the Smart Era UI repository — the "Phase 2" item from `docs/spec/strategy/2026-09-plan.md` (§4, P2.1). It is not a benchmark of Smart Era UI's components in isolation; it is a benchmark of what an agent produces when it has Smart Era UI available versus when it has a documented shadcn/ui baseline available, given the same prompt and the same model.
...
Five "golden tasks" — a settings page, a data table, a pricing page, a multi-step onboarding flow, and a dashboard — each run twice per model: once with Smart Era UI installed, once with the documented shadcn/ui baseline installed. Same prompt, same model, same viewport list, different starting library.
...
2. Install either Smart Era UI (`npx smarteraui@latest init`) or the shadcn/ui baseline, per `baseline/README.md`.
...
4. **No prompt engineering per condition.** The prompt in `tasks/` is what gets pasted, verbatim, for both the Smart Era UI run and the shadcn/ui run. If you think a prompt is unfair to one side, open an issue to fix the task file — don't patch it silently for one run.
...
Open a PR that adds one file under `results/` following `results/TEMPLATE.md`, and — if your numbers are strong enough that you want them reflected in the table — a row in `leaderboard.md`. A maintainer reviews for the honesty rules above before merging, not for whether the result looks good for Smart Era UI.
