...
`smarteraui-docs`, serving **https://smarteraui.com**.
...
`/r/*` is the public component registry the `smarteraui` CLI talks to
(`DEFAULT_REGISTRY_URL = https://smarteraui.com/r`), so the CLI works against the live
...
2. The `smarteraui.com` domain, registered anywhere.
...
1. Cloudflare dashboard → **Add a site** → `smarteraui.com` → pick a plan (Free is fine).
...
- **Dashboard:** Workers & Pages → `smarteraui-docs` → Settings → Domains & Routes →
  **Add** → Custom domain → `smarteraui.com`. Repeat for `www.smarteraui.com`.
...
Either way Cloudflare creates proxied (orange-cloud) `CNAME` records for `smarteraui.com`
and `www.smarteraui.com` pointing at the Worker, and issues the TLS certificate. If the
...
`smarteraui.com` as a custom domain and add a Cloudflare **Redirect Rule**
(`www.smarteraui.com/*` → `https://smarteraui.com/$1`, 301).
...
| ----------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `CLOUDFLARE_API_TOKEN`  | My Profile → API Tokens → Create Token → **Edit Cloudflare Workers** template. Scope it to this account and to the `smarteraui.com` zone. It needs Workers Scripts:Edit, Account Settings:Read, User Memberships:Read, and — once custom domains are on — Zone Workers Routes:Edit and Zone DNS:Edit. |
...
pnpm build:packages      # @smarteraui/ui + smarteraui (CLI)
...
`https://smarteraui-docs.<your-subdomain>.workers.dev`.

Sanity checks after the first deploy:

```bash
curl -sI  https://smarteraui.com/                  # 200
curl -s   https://smarteraui.com/robots.txt        # sitemap points at smarteraui.com
curl -s   https://smarteraui.com/r/index.json | head
npx smarteraui@latest list                         # CLI resolves the public registry
```
...
Or in the dashboard: Workers & Pages → `smarteraui-docs` → **Deployments** → pick a past
...
- **`NEXT_PUBLIC_SITE_URL`** overrides the canonical origin (`lib/site.ts` defaults to
  `https://smarteraui.com`). Set it as a Worker var only if the site moves.
