...
```bash
npx properui@latest add badge-groups
```
...
```bash
npx properui@latest add --all
```

Customize the installation directory:

```bash
npx properui@latest add button --path src/components/ui
```
...
```bash
npx properui@latest add example hero-split-image-01
```
...
```bash
npx properui@latest list --type example
```
...
npx properui@latest add button input select badges avatar
```
...
```bash
npx properui@latest search "hero split image"
npx properui@latest add example hero-split-image-01
npx properui@latest add example hero-split-image-01 --overwrite
```
...
<FAQs
    items={[
        ...
        },
        {
            ...
            answer: "Only for a private registry of your own. Run npx properui@latest login and paste a token when prompted, or pass --token directly — for example in CI. The token is saved to ~/.properui/auth.json and reused by every later command on the same machine; delete that file to log out.",
        },
        {
        ...
        },
        {
            ...
            answer: "Yes, you can use the --path flag to specify where components should be installed. For example: npx properui@latest add button --path src/components/ui.",
        },
        {
        ...
        },
        {
            ...
            answer: "You can re-run the add command with the --overwrite flag to update components to their latest versions: npx properui@latest add button --overwrite.",
        },
    ]}
/>
