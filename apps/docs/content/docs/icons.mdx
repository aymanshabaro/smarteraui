...
`@smarteraui/icons` is a published alias of [`@untitledui/icons`](https://www.npmjs.com/package/@untitledui/icons) — installing it gets you that package under the `@smarteraui/icons` name, so its README and its 1,100+ icons apply directly here.

## Installation

```bash
npm install @smarteraui/icons
```

```bash
pnpm add @smarteraui/icons
```

`@smarteraui/icons` is already listed as a dependency of `@smarteraui/ui`, so if you installed the component library you have it. Installing it directly only matters if you're using icons standalone, outside of any Smartera UI component.
    ...
    items={[
        {
            ...
            answer: "Smartera UI Icons are a clean, consistent, and neutral icon library crafted specifically for modern UI design, distributed as tree-shakeable React components with instant npm access. The package is a published alias of @untitledui/icons.",
        },
        {
