    ...
    items={[
        {
            ...
            answer: "An MCP server doesn't exist for Proper UI today, but a Skill file does, in the .mdc rule format Cursor reads — run npx @properui/cli@latest agent init --client cursor to install it. Combined with Cursor's own terminal tool, that's enough; Agent mode can already run npx @properui/cli@latest search and add.",
        },
        {
