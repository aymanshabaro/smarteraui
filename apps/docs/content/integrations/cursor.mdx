...
description: Point Cursor at Proper UI's registry with a project rule installed in one command, so Agent mode composes screens from real components instead of inventing markup.
metaTitle: Cursor integration | Proper UI
metaDescription: Use Proper UI with Cursor — install the Proper UI rule with one command, and prompts that resolve to real components.
---
...
- A project already scaffolded with `npx properui@latest init` — Cursor works best once `components.json` exists.
...
Rather than writing `.cursor/rules/properui.mdc` by hand, install it with the [Proper UI Skill](/docs/agents) command:
    ...
    items={[
        {
            ...
            answer: "An MCP server doesn't exist for Proper UI today, but a Skill file does, in the .mdc rule format Cursor reads — run npx properui@latest agent init --client cursor to install it. Combined with Cursor's own terminal tool, that's enough; Agent mode can already run npx properui@latest search and add.",
        },
        {
