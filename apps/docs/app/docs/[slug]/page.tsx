...
export const generateStaticParams = () => getContentPages("docs").map((page) => ({ slug: page.slug }));

export const generateMetadata = async ({ params }: Params): Promise<Metadata> => {
    const { slug } = await params;
    const page = getContentPage("docs", slug);
...
export default async function DocsSlugPage({ params }: Params) {
    ...
    const mdx: MdxModule | undefined = await import(`../../../content/docs/${slug}.mdx`).catch(() => undefined);
    if (!mdx) notFound();
    ...
    return (
        <DocPage area="docs" slug={slug} frontmatter={frontmatter} source={stripFrontmatter(readMdxSource("docs", slug) ?? "")}>
            <MdxContent />
        </DocPage>
    );
}
